...
class AddToHomeScreen {
	...
	public function add_overlays() {	
		if ( wp_is_mobile() ) {
		    ...
		    $cib     = __( 'در مرورگر استفاده میکنم', 'pwa' );
		    $tit     = __( 'روی علامت', 'pwa' );
		    $ac      = __( 'کلیک کنید و سپس', 'pwa' );
		    $aths    = __( 'گزینه Add to Home Screen انتخاب کنید', 'pwa' );
		    $rs      = __( 'بازگردانی صفحات', 'pwa' );
		    $rspg    = __( 'آخرین صفحاتی که بررسی کردی رو میخوای بازیابی کنیم؟', 'pwa' );
		    $restore = __( 'آره', 'pwa' );
		    $cancel  = __( 'نه', 'pwa' );
