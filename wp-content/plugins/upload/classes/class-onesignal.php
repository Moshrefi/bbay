...
class OneSignal {
	...
	public function run() {
		...
		add_filter( 'pwa_manifest_values', [ $this, 'manifest_values' ] );
		add_filter( 'pwa_sw_content', [ $this, 'get_sw_content' ] );
		add_action( 'wp_loaded', function () {
			do_action('pwa_onesignal_active');
			remove_action( 'wp_head', [ 'OneSignal_Public', 'onesignal_header' ] );
		} );
		add_action( 'wp_head', [ $this, 'init_onesignal' ] );

		register_deactivation_hook( pwa_get_instance()->file, function () {
	...
	public function set_os_settings() {
		...
		$os_settings['custom_manifest_url'] = esc_url( pwa_get_instance()->AddToHomeScreen->get_manifest_url(false) );
		\OneSignal::save_onesignal_settings( $os_settings );
		$this->os_settings = \OneSignal::get_onesignal_settings();
	}
	...
	public function get_sw_content( $content ) {
		return $content . "importScripts('https://cdn.onesignal.com/sdks/OneSignalSDKWorker.js');";
	}

	public function init_onesignal() {
		?>
		<link rel="manifest" href="<?php echo $this->os_settings['custom_manifest_url'] ?>"/>
		...
		<script>
			...
			OneSignal.push(function () {
				...
				OneSignal.SERVICE_WORKER_UPDATER_PATH = "<?php echo esc_url( pwa_get_instance()->ServiceWorker->get_sw_url(false) ) ?>";
				OneSignal.SERVICE_WORKER_PATH = "<?php echo esc_url( pwa_get_instance()->ServiceWorker->get_sw_url(false) ) ?>";
